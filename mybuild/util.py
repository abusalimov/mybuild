...
class DynamicAttrsMeta(type):
    ...
    def __init__(cls, name, bases, attrs):
        super(DynamicAttrsMeta, cls).__init__(cls, name, bases, attrs)
        cls._dam_registered_attrs = {} # attr-to-method
    ...
    def register_attr(cls, attr, factory_method='_new_type'):
        ...
        def deco(target):
            ...
            if hasattr(cls, attr) or attr in cls._dam_registered_attrs:
                raise ValueError("%s class already has attribure '%s'" %
                                 (cls.__name__, attr))

            cls._dam_registered_attrs[attr] = getattr(target, factory_method)

            return target

        return deco

    def _create_attrs(cls, *args, **kwargs):
        meta = type(cls)
        for base in cls.__mro__:
            if isinstance(base, meta):
                for attr, factory in base._dam_registered_attrs.iteritems():
                    yield attr, factory(*args, **kwargs)
