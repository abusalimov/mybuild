...
if (not got_tty and os.environ.get('TERM', 'dumb') != 'msys'):
...
def nesting_log(log=None, *k):
    ...
    try:
        ...
        if log is not None:
            log('%s: raised %r' % (k[0].split(':')[0], e))
        raise e
    finally:
        nesting_level -= 1

def with_nesting(fxn):
    @functools.wraps(fxn)
    def wrapper(*k, **kw):
        fxn(*k, **kw)
        return nesting_log(fxn, *k)
    return wrapper
